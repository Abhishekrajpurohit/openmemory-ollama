...
def get_memory_client(custom_instructions: str = None):
    ...
    try:
        ...
        # Load configuration from JSON files first (highest priority)
        try:
            config_paths = [
                "/usr/src/openmemory/config.json",
                "/usr/src/openmemory/default_config.json"
            ]
                ...
                if os.path.exists(config_path):
                    ...
                    if "mem0" in json_config:
                        ...
                        if "llm" in mem0_config and mem0_config["llm"] is not None:
                            ...
                            print(f"Loaded LLM config: {config['llm']}")
                        ...
                        if "embedder" in mem0_config and mem0_config["embedder"] is not None:
                            ...
                            print(f"Loaded embedder config: {config['embedder']}")
            ...
            print(f"Warning: Error loading configuration from JSON files: {e}")
        
        # Load configuration from database (lower priority than files)
        try:
            db = SessionLocal()
